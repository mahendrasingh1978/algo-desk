...
print("\n25. Market data feeds (VIX, prev_close, gap guards)...")

def t_dashboard_has_vix_field():
    """Dashboard API must return vix and prev_close fields (even if 0 — no broker in test)."""
    r = client.get("/api/dashboard/summary", headers=H())
    ok_status(r)
    d = r.json()
    assert "vix" in d, "Dashboard missing 'vix' field"
    assert "prev_close" in d, "Dashboard missing 'prev_close' field"
    assert isinstance(d["vix"], (int, float)), f"vix must be numeric, got {type(d['vix'])}"
    assert isinstance(d["prev_close"], (int, float)), f"prev_close must be numeric"
test("Dashboard: vix and prev_close fields present", t_dashboard_has_vix_field)

def t_vix_guard_fires_when_set():
    """Guard 2: if vix_open >= vix_max, check_all_strategies returns None."""
    from engine import EngineState, StrikeState, check_all_strategies
    from datetime import datetime, time as dtime
    s = EngineState({"mode": "paper", "vix_open": 21.5, "vix_max": 17.0})
    s.orb_complete = True; s.spot_locked = 23100.0; s.spot_history = [23100]
    s.atm_strike = 23100
    for i in range(-3, 4):
        sk = StrikeState(strike=23100 + i * 50, offset=i, is_atm=(i == 0))
        sk.combined_history = [480.0] * 25
        s.strikes.append(sk)
    now = datetime.now()
    sig = check_all_strategies(s, now)
    assert sig is None, "VIX Guard should block all signals when vix_open >= vix_max"
    assert "VIX" in (s.guard_status or ""), f"guard_status should mention VIX, got: {s.guard_status}"
test("Guard 2: VIX filter blocks all signals when vix_open >= vix_max", t_vix_guard_fires_when_set)

def t_vix_guard_skips_when_zero():
    """Guard 2: when vix_open=0 (not fetched), guard must not block."""
    from engine import EngineState, check_all_strategies
    from datetime import datetime
    s = EngineState({"mode": "paper", "vix_open": 0, "vix_max": 17.0})
    # VIX=0 means data unavailable — guard must silently pass (existing behaviour)
    assert s.config.get("vix_open", 0) == 0
    # Guard fires only if vix_open AND vix_open >= vix_max
    vix_open = s.config.get("vix_open", 0)
    would_block = bool(vix_open and vix_open >= s.config.get("vix_max", 17.0))
    assert not would_block, "Guard 2 must NOT block when vix_open=0 (data unavailable)"
test("Guard 2: VIX=0 (unavailable) does not block trading", t_vix_guard_skips_when_zero)

def t_s8_skips_without_prev_close():
    """S8 gap fade must return None when prev_close=0 (correct silent skip)."""
    from engine import EngineState, StrikeState, _s8
    from datetime import datetime, time as dtime
    s = EngineState({"mode": "paper", "prev_close": 0})
    s.atm_strike = 23100; s.spot_locked = 23250.0; s.spot_history = [23250]
    s.orb_complete = True
    for i in range(-4, 5):
        sk = StrikeState(strike=23100 + i * 50, offset=i, is_atm=(i == 0))
        sk.combined_history = [480.0] * 5
        sk.ce_symbol = f'NIFTY{23100+i*50}CE'
        sk.pe_symbol = f'NIFTY{23100+i*50}PE'
        s.strikes.append(sk)
    sig = _s8(s, dtime(9, 35), datetime.now())
    assert sig is None, "S8 must skip when prev_close=0"
test("S8: skips gracefully when prev_close not available", t_s8_skips_without_prev_close)

def t_s8_fires_with_prev_close():
    """S8 gap fade should fire on a genuine gap day when prev_close is populated."""
    from engine import EngineState, StrikeState, _s8
    from datetime import datetime, time as dtime
    # 0.8% gap up — within S8 range (0.5–2%) and prev move <2%
    s = EngineState({"mode": "paper", "prev_close": 23100.0, "prev_day_move_pct": 1.0})
    s.atm_strike = 23250; s.spot_locked = 23285.0; s.spot_history = [23285]
    s.orb_complete = True
    for i in range(-5, 6):
        sk = StrikeState(strike=23250 + i * 50, offset=i, is_atm=(i == 0))
        sk.combined_history = [490.0] * 25
        sk.orb_high = 500.0; sk.orb_low = 480.0
        sk.ce_symbol = f'NIFTY{23250+i*50}CE'
        sk.pe_symbol = f'NIFTY{23250+i*50}PE'
        s.strikes.append(sk)
    sig = _s8(s, dtime(9, 36), datetime.now())
    # Signal fires OR returns None due to ORB conditions — just verify no crash
    # and no AttributeError from missing prev_close
    assert sig is None or sig.get("code") == "S8", f"Unexpected signal: {sig}"
test("S8: no crash when prev_close is populated", t_s8_fires_with_prev_close)

def t_user_market_cache_vix_structure():
    """user_market_cache must accept vix, prev_close, prev_day_move_pct fields."""
    import main as m
    uid = "test-vix-cache-uid"
    m.user_market_cache[uid] = {
        "spot": 23100.0, "atm": 23100, "chain": {},
        "updated": "09:16:00", "status": "live", "message": "Live",
        "vix": 14.25, "prev_close": 23000.0,
        "prev_day_move_pct": 1.2, "vix_date": "2026-03-26",
    }
    cached = m.user_market_cache[uid]
    assert cached["vix"] == 14.25, "VIX not stored in cache"
    assert cached["prev_close"] == 23000.0, "prev_close not stored"
    assert cached["prev_day_move_pct"] == 1.2, "prev_day_move_pct not stored"
    assert cached["vix_date"] == "2026-03-26", "vix_date not stored"
    # Verify preserved through simulated cache refresh
    _prev = m.user_market_cache.get(uid, {})
    refreshed = {
        "spot": 23150.0, "atm": 23150, "chain": {},
        "updated": "09:17:00", "status": "live", "message": "Live",
        "vix":              _prev.get("vix", 0),
        "prev_close":       _prev.get("prev_close", 0),
        "prev_day_move_pct": _prev.get("prev_day_move_pct", 0),
        "vix_date":         _prev.get("vix_date", ""),
    }
    assert refreshed["vix"] == 14.25, "VIX lost during cache refresh"
    assert refreshed["prev_close"] == 23000.0, "prev_close lost during cache refresh"
    del m.user_market_cache[uid]
test("Market cache: vix/prev_close persisted across refresh ticks", t_user_market_cache_vix_structure)
